...
The chosen routes are in comparison shorter, less steep...
...
::: {#fig-segment_length_messenger layout-ncol="1"}
![](figures/seg_length_mess.png){.lightbox}

Difference in segment lengths by messengers and route type
...
Difference in segment lengths by all route types
...
slope_result <- slope |> 
  ...
  summarise(
    "zwei_bis_sechs" = mean(mean_slope_up >= 2 & mean_slope_up < 4),
    "sechs_bis_zehn" = mean(mean_slope_up >= 4 & mean_slope_up < 6),
    "mehr_als_10" = mean(mean_slope_up >=6)
      )

sloppy <- pivot_longer(slope_result, -c(type, segment_id))

sloppy$log <- log10(sloppy$value)


ggplot(sloppy, aes(x = name, y = value, fill = type)) +
  geom_boxplot() +
  scale_y_log10()
...
lm = aov(value ~ type+name, data = sloppy)
