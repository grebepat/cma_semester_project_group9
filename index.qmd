...
segment_endpoints <- segment_endpoints %>%
  mutate(
    end_in_entry_buffer = st_intersects(end_point, buffer_entry, sparse = FALSE),
    start_in_exit_buffer = st_intersects(start_point, buffer_exit, sparse = FALSE)
  )
...
load("rda_files/alternative.rda")
...
ggplot(df_spread_original, aes(x = messenger, y = tot_travel, fill = messenger,   alpha = 0.75)) +
  geom_boxplot() +
...
ggplot(df_spread, aes(x = messenger, y = tot_travel, fill = type,   alpha = 0.75)) +
  geom_boxplot() +
   labs(x = "segment type", y = "total segment length") +
  scale_fill_manual(values = c("#11E2A1","#9E1FA2")) +
   theme_classic() +
...
ggplot(comp, aes(x = name, y = value, fill = name,   alpha = 0.75)) +
  geom_boxplot() +
   labs(x = "segment type", y = "total segment length") +
  scale_fill_manual(values = c("#11E2A1","#9E1FA2")) +
   theme_classic() +
...
t.test(comparison1$original, comparison1$alternative1, paired = T)
...
# mean difference between original route and shortest generated route at 31.8m # in 40% of the times, original was still shortest! so basically no difference between shortest route generated and choosen route!


# So we see slight differenc ebetween messengers, but also that alternatives and originals differ. Alternative 1 does not, but overall does, we have to take a look at this..
...
## Keine signifikante Interaktion zwischen rank und messenger, daher Interaktion raus und alles nochmals laufen lassen
...
## Very interesting, our multifacotrial anova is significant, we see that messenegers and ranks differ significantly. But we also se


# mit jitterpunkten


ggplot(df_spread, aes(x = rank, y = tot_travel, fill = type)) +
 geom_jitter(aes(colour = tot_travel)) +
  geom_boxplot(alpha = 0.75) +
   labs(x = "trajectory type", y = "total segment length") +
  scale_fill_manual(values = c("#11E2A1","#9E1FA2")) +
...
## visualize anova
ggplot(df_spread, aes(x = rank, y = tot_travel, fill = type,  alpha = 0.75)) +
  geom_boxplot() +
   labs(x = "segment type", y = "total segment length") +
  scale_fill_manual(values = c("#11E2A1","#9E1FA2")) +
   theme_classic() +
...
spread$width <- factor(spread$width, levels = spread$width,
                       labels = c("1m Strasse", "6m Strasse", "8m Strasse",
                                  "4m Strasse", "2m Strasse", "3m Strasse"))


ggplot(spread, aes(x = width, y = diff)) +
...
ggplot(slope, aes(x=mean_slope_up, fill= type)) +
...
ggplot(slope, aes(x=mean_slope_up, fill= type)) +
  geom_histogram(alpha = 0.4, binwidth = 0.25) +
 theme_minimal()


# slope$rank <- factor(slope$rank, labels = c("Original", "A1", "A2", "A3", "A4", "A5", "A6", "A7"))


# ggplot(slope, aes(x=max_slope_up, fill= type)) +
#   geom_density(alpha = 0.75) +
#    scale_fill_manual(values =c("#11E2A1","#9E1FA2")) +
#  theme_minimal()
# 
# 
# ggplot(slope_vis, aes(x=max_slope_up, y = type)) +
#   geom_density_ridges(aes(fill = type),alpha = 0.75) +
#    scale_fill_manual(values =c("#11E2A1","#9E1FA2")) +
#  theme_minimal()


slope$rank <- factor(slope$rank, levels = c("Original", "A1", "A2", "A3", "A4", "A5", "A6", "A7"))
...
ggplot(slope_vis, aes(x = mean_slope_up, y = rank, height = ..density..)) +
  geom_density_ridges(aes(fill = rank), alpha = 0.75, stat = "density") +
  scale_fill_manual(values =c("#9E1FA2","#11E2A1","#11E2A1","#11E2A1","#11E2A1","#11E2A1","#11E2A1","#11E2A1")) +
 theme_minimal() +
  theme(
    legend.position = "none"
  )
...
ggplot(slope_vis2, aes(x = max_slope_up, y = rank, height = ..density..)) +
  geom_density_ridges(aes(fill = rank), alpha = 0.75, stat = "density") +
  scale_fill_manual(values =c("#9E1FA2","#11E2A1","#11E2A1","#11E2A1","#11E2A1","#11E2A1","#11E2A1","#11E2A1")) +
 theme_minimal() +
  theme(
