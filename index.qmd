...
We used the HERE API to generate alternative routes. The hereR[^4] package provides an interface to the HERE REST APIs for R. The route function from the hereR package was used to generate alternative routes and enabled the calculation of a maximum of 7 alternative routes per original delivery. Based on the starting and endpoints of the corrected deliveries @fig-original_route ´route()´ enables calculation of route geometries (LINESTRING) between given pairs of points @fig-alternatives. Routes can be created for various transport modes, as for example 'car' or 'bicycle'. As this step needs a lot of time, the computation was transfered to `preprocessing.qmd`. The final result consist of the original route combined with one to seven alternative routes @fig-final_result.
...
Difference in segment lengths by original and shortest alternative

:::

### Street width
Next, we investigated whether certain types of streets were preferred by the messengers based on street width. Since there is no clear procedure for evaluating preferences, we developed an own approach.In each segment, we calculated the share of the different street widths (1, 2, 3, 4, 6, and 8 meters wide) in contrast to the segment length. For each segment (original and alternative) we get percentages on how much a certain street width was used. We then combined the calculated percentage values for all alternatives per segment. For each segment, we obtained a single alternative that summarizes the shares based on street width (see output tibble). This average alternative can then be compared with the actual route taken by the messengers. 

This approach reveals clear preferences or aversions among three street widths. Narrow streets (1 meter wide) are clearly avoided by bike messengers compared to the average alternative, while streets two to three meters wide are clearly preferred @fig-preference.
...
::: {layout-nrow="2"}
![mean bebe](figures/mean_slope_up.png){.lightbox}

![max bebe](figures/max_slope_up.png){.lightbox}
...
machen Höhenunterschiede / Slope überhaupt viel Sinn bei routen die in einem so kleinen spatial context konentriert sind? Es gibt de fakto keine Möglichkeiten den Uetli- oder Züroiberg zu Umfahren, die Messengers aber auch alternativen sind gezwungen die gleichen Höhenunterschiede zu fahren. Vielleicht macht es hier mehr sinn in einen grsseren spatial context zu gehen, wo hügel umfahren werden können?
