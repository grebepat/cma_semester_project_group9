...
seg_length <- ggplot(df_spread_original, aes(x = messenger, y = tot_travel, fill = messenger,   alpha = 0.75)) +
  geom_boxplot() +
...
mean_slope_up <-  ggplot(slope_vis, aes(x = mean_slope_up, y = rank1, height = ..density..)) +
 ...
 theme_minimal() +
    labs(
    x = "mean positive slope over 100 m subsegments [%]"
  ) +
  theme(
    legend.position = "none",
    axis.title.x = element_text(margin = margin(t = 20)),
...
scale_fill_manual(values =c("#11E2A1","#11E2A1","#11E2A1","#11E2A1","#11E2A1","#11E2A1","#11E2A1","#9E1FA2")) +
 theme_minimal() +
   labs(
    x = "max positive slope over 100 m subsegments [%]") +
  theme(
    legend.position = "none",
    axis.title.x = element_text(margin = margin(t = 20)),
    axis.title.y = element_blank()
...
slope_category <- ggplot(slope_final, aes(x = name, y = mean_count, fill = type,   alpha = 0.75)) +
  geom_boxplot() +
  scale_y_log10() +
   labs(y = "mean count per segment (Log Scale)",
        x = "slope categories") +
  scale_fill_manual(values = c("#11E2A1","#9E1FA2")) +
   theme_classic() +
  theme(
